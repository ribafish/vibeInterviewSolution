...
- Run: `./gradlew -p examples/multi-project projectReport` (composite build via `includeBuild("..")` to source the plugin). Build scans are auto-published to `scans.gradle.com` via the example’s `settings.gradle.kts`, and configuration cache is enabled by default (`gradle.properties`).
