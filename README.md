...
- Run: `./gradlew -p examples/multi-project projectReport` (composite build via `includeBuild("..")` to source the plugin). Build scans are auto-published to `scans.gradle.com` via the example’s `settings.gradle.kts`.
